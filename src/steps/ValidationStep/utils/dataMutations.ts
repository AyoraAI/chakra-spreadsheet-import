...
export const addErrorsAndRunHooks = async <T extends string>(
  ...
  if (tableHook) {
    data = await tableHook(data, (...props) => {
      addError(ErrorSources.Table, ...props)
    })
  }
    ...
    if (changedRowIndexes) {
      for (const index of changedRowIndexes) {
        data[index] = await rowHook(
          data[index],
          (...props) => {
            addError(ErrorSources.Row, index, ...props)
          },
          data,
        )
      }
  ...
  fields.forEach((field) => {
    field.validations?.forEach((validation) => {
      switch (validation.rule) {
        ...
        case "required": {
          const dataToValidate = changedRowIndexes
            ? changedRowIndexes.map((index) => data[index])
            : data
          dataToValidate.forEach((entry, index) => {
            const realIndex = changedRowIndexes
              ? changedRowIndexes[index]
              : index
            if (!entry[field.key as T]) {
    ...
    // if we have not validated this row, keep it's row errors but apply global error changes
      ...
      const hasRowErrors =
        ...
        Object.values(value.__errors).some(
          (error) => error.source === ErrorSources.Row,
        )
